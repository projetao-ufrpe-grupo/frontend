...
import { BookOpen, Building2, Mail, MapPin, MessageSquare, Pencil, School } from "lucide-react"
...
export default function ProfilePage() {
  ...
  const [privacidade, setPrivacidade] = useState<PrivacidadePerfil>("PUBLICO")
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setUser(userInfo)
        setPrivacidade(response.privacidadePerfil)
      } catch (error) {
  ...
  const handlePrivacyChange = async (value: PrivacidadePerfil) => {
    try {
      setIsLoading(true)
      await authService.atualizarPrivacidade(value)
  ...
  const privacyOptions = [
    ...
    { value: 'APENAS_LOCADORES', label: 'Apenas Locadores' }
  ]
    ...
    <div className="min-h-screen">
      ...
      <div className="container py-8">
        <div className="mb-4 flex items-center gap-4">
          <Select
            value={privacidade}
            onValueChange={(value) => handlePrivacyChange(value as PrivacidadePerfil)}
            disabled={isLoading}
          >
          ...
          <p className="text-sm text-muted-foreground">
            {privacidade === 'PUBLICO' && 'Seu perfil está visível para todos'}
            {privacidade === 'PRIVADO' && 'Seu perfil está visível apenas para você'}
            {privacidade === 'APENAS_LOCADORES' && 'Seu perfil está visível apenas para locadores'}
          </p>
        </div>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-muted-foreground text-center py-8">
                  Você não possui anúncios no momento.
                </p>
              </CardContent>
            </Card>
          </TabsContent>
